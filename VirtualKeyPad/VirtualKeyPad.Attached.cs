...
namespace VirtualKeyPad
{
	public partial class VirtualKeyPad
	{
		...
		public static readonly DependencyProperty ModeProperty =
			DependencyProperty.RegisterAttached("Mode", typeof(VirtualKeyPadMode), typeof(VirtualKeyPad),
				new PropertyMetadata(VirtualKeyPadMode.Disabled, OnModeChanged));
		#endregion
		...
		public static VirtualKeyPadMode GetMode(DependencyObject obj)
		{
			return (VirtualKeyPadMode) obj.GetValue(ModeProperty);
		}

		public static void SetMode(DependencyObject obj, VirtualKeyPadMode value)
		{
		...
		private static void OnModeChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
		{
			...
			var mode = (VirtualKeyPadMode) e.NewValue;
			RegisterAttachedControl(attachedControl, mode);
		}

		private static void RegisterAttachedControl(UIElement attachedControl, VirtualKeyPadMode mode)
		{
			if (attachedControl != null && mode != VirtualKeyPadMode.Disabled)
			{
				SetIsKeyPadEnabled(attachedControl, true);	//flag for binding

				if (mode.HasFlag(VirtualKeyPadMode.Touch))
				{
					attachedControl.IsManipulationEnabled = true;
					attachedControl.ManipulationStarted += AttachedControlOnManipulationStarting;
				}
				if (mode.HasFlag(VirtualKeyPadMode.Mouse))
				{
			...
			if (mode == VirtualKeyPadMode.Disabled)
			{
